...
class GreedyTreeInvariants(Invariants):
    ...
    def is_invariants_violated(self, new_statistics: dict, pattern: Pattern):
        if (
            StatisticsTypes.ARRIVAL_RATES in new_statistics
            and StatisticsTypes.SELECTIVITY_MATRIX in new_statistics
            and len(new_statistics) == 2
        ):
        ...
        for i in range(len(self.invariants) - 1):
            ...
            left_change_factor = (
                selectivity_matrix[left_index][left_index] * arrival_rates[left_index]
            )
            right_change_factor = (
                selectivity_matrix[right_index][right_index]
                * arrival_rates[right_index]
            )

            for j in range(i):
                left_change_factor *= selectivity_matrix[left_index][
                    self.invariants[j].left
                ]
                right_change_factor *= selectivity_matrix[right_index][
                    self.invariants[j].left
                ]
...
class ZStreamTreeInvariants(Invariants):
    ...
    def is_invariants_violated(self, new_statistics: dict, pattern: Pattern):
        for invariant in self.invariants:
            left_cost = self.__get_plan_cost_callback(
                pattern, invariant.left, new_statistics
            )
            right_cost = self.__get_plan_cost_callback(
                pattern, invariant.right, new_statistics
            )
